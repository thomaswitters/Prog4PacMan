...
namespace ImGui {
// [0..1] -> [0..1]
static double rescale(double t, double min, double max, PlotConfig::Scale::Type type) {
    switch (type) {
...
// [0..1] -> [0..1]
static double rescale_inv(double t, double min, double max, PlotConfig::Scale::Type type) {
    switch (type) {
    case PlotConfig::Scale::Linear:
        return t;
    case PlotConfig::Scale::Log10:
...
static int cursor_to_idx(const ImVec2& pos, const ImRect& bb, const PlotConfig& conf, double x_min, double x_max) {
    const double t = ImClamp((pos.x - bb.Min.x) / (bb.Max.x - bb.Min.x), 0.0f, 0.9999f);
